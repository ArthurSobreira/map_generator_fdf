...
def parse_without_color(image, apply_log: bool):
...
def	get_input_log(text: str):
	while True:
		response = input(text).lower().strip()
		if response in ['y', 'yes']:
			return True
		elif response in ['n', 'no']:
			return False
		else:
			print('\033[31mInvalid input. Please enter yes/y or no/n\033[m')
...
def main():
	...
	apply_log = get_input_log('''Apply logarithmic scaling to height values?\n
  (recommended if you can't modify Z scale in the fdf) (y/n): ''')
	...
	if use_default_color:
		relief_map = parse_without_color(image, apply_log)
